...
```{r, include =F}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, tidy = TRUE, cache= T)
```
...
nested_pe_df %>% 
  ggplot(aes(x = simu_id, y = dre)) +
  geom_point(color = 'red') +
  ggtitle(paste('The mean of the DRE estimate (IPW misspecified) is ', round(mean(nested_pe_df$dre), 2))) + 
  geom_hline(yintercept = 10) +ylab("ACE estimate")+
  theme_bw()
```
...
nested_om_df %>% 
  ggplot(aes(x = simu_id, y = dre)) +
  geom_point(color = 'red') +
  ggtitle(paste('The mean of the DRE estimate (REG misspecified) is ', round(mean(nested_om_df$dre), 2))) + 
  geom_hline(yintercept = 10) +ylab("ACE estimate")+
  theme_bw()
```
...
nested_bw_df %>% 
  ...
  geom_point(color = 'red') +
  ggtitle(paste('The mean of the DRE estimate (both incorrect) is ', round(mean(nested_bw_df$dre_w), 2))) + 
  geom_hline(yintercept = 10) +ylab("ACE estimate")+
  theme_bw()
```

```{r}
summary(nested_df$dre)
summary(nested_pe_df$dre) #IPW wrong
summary(nested_om_df$dre)
summary(nested_bw_df$dre_w) 
```


```{r, echo=F}
# test git
h <- ggplot(nested_df, aes(x=simu_id, y=dre, group=1))+
  geom_boxplot()+
...
plot_grid(h, e, f, g, labels=c("Both Correct", "Incorrect IPW", "Incorrect REG", "Both Incorrect"), ncol = 4, nrow = 1)
```
